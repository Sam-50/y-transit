...
async function getParentData(email) {
    ...
    const studentWithParent = students.find((student)=>student.parent.email === email);
    if (studentWithParent) {
        return studentWithParent.parent;
    }
    return null;
}
const getUserRole = async (email)=>{
    const driver = await getDriverData(email);
    if (driver) {
        return 'driver';
    }
    const parent = await getParentData(email);
    if (parent) {
        return 'parent';
    }
    // If not a driver or parent, assume admin.
...
const getUserData = async (email)=>{
    ...
    // Default to admin if no other role is found
    return {
        name: 'Admin User',
        email,
        role: 'admin'
...
function DashboardLayout({ children }) {
                ...
                "DashboardLayout.useEffect.unsubscribe": async (user)=>{
                    if (user && user.email) {
                        try {
                            ...
                            if (data) {
                                // Check if the current URL matches the user's role
                                if (pathname.startsWith(`/dashboard/${data.role}`)) {
                                    setUserData(data);
                                } else {
                                    // Redirect if the role does not match the dashboard path
    ...
    const fallback = userData ? getFallback(name) : 'U';
    if (isLoading) {
        return /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
...
_s3(DashboardLayout, "AxAuaLp7X6YSAyTb+/eFtuvbXuw=", false, function() {
    return [
        __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$navigation$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["usePathname"],
...
//# sourceMappingURL=src_e4c66200._.js.map